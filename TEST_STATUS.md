...
- Normal-play semantics: blank slots, 64-line cap, case-insensitive names, side mismatch, special-in-normal rejection, duplicate names — every violation type raises `InvalidPlayInputError` with the matching message and leaves the target `.pln` unchanged
- Special-play semantics: self-slotting by category, blank skipping — unknown names, normal-in-special rejection, side mismatch, duplicate name, and duplicate category all raise `InvalidPlayInputError`
